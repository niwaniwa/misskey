...
## 2024.9.0 (merged to 2024.10.0-kinel.1)
...
### Server
- Feat: ユーザー名の前後に空白文字列がある場合も省略しないように
- Feat: プロフィール編集時に名前を空白文字列のみにできるように

## 2024.7.0 (merged to 2024.7.0-kinel.1)
...
### Note
- 2023.9.3-kinel.4以前で利用されていたソフト/ハードミュートの設定は、そのまま2023.11.1-kinel.1向けに移行されます。
  - 本来、2023.10.0の変更により以下の事象が発生する可能性がありました。
    - ソフトミュートの設定がリセットされる
    - ハードミュートの設定はソフトミュートのものになり、ハードミュート機能が一時廃止
  - 本更新では、ハードミュート機能を再実装し、2023.9.3-kinel.4以前からの更新時に設定が正常に引き継げるようになっています。
...
### Client
- Fix: Renoteの省略表示でセンシティブチャンネルの自動CWが効かない問題

### Server
- 2023.10.x向けのTLを内部的に構築するようになりました
  - これにより、2023.9.3-kinel.4更新後のnoteは2023.10.x以降に更新した後でも見ることが出来ます
  - 2023.10.xには、更新以前のnoteをTLで見ることが出来ないという仕様がありました
  - 2023.9.xを利用している内は特に影響ありません
...
- Fix: "右クリックでリアクションピッカーを開くようにする"が2つある
...
- センシティブチャンネルのNoteがユーザページからたたまれるのを無効化できるようにしました
